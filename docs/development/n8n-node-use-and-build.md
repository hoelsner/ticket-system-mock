...
3. run `make n8n-node-stage-dev-package` and validate the node in the local n8n
   development instance when the change affects runtime behavior
4. run `make n8n-node-pack` if the package artifact or distribution path changed
5. run repository validation such as `make check` and `make test` when the
   change affects shipped behavior
6. if the change affects runtime installation or delivery, verify the webapp
   image still contains the packaged file under `/app/build/integrations`
7. update the relevant n8n reference docs under `docs/external/n8n/` and the
